...
def inference_engine_loader(engine, inference_engine_dict, status):
	...
	with engine.connect() as link_to_db:
		...
		if q.rowcount > 0 :
			query = f"UPDATE {table_name} SET current_flag={status}, updated_date={datetime.now().replace(tz=None)}, updated_by={inference_engine_dict['created_by']} WHERE created_by={inference_engine_dict['created_by']}"
		else :
			q = link_to_db.execute(f"SELECT max(seq) from {table_name}")
			if q.rowcount > 0 :
